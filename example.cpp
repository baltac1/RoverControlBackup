...
TalonSRX talLeftFrnt(0);
TalonSRX talRghtFrnt(1);
TalonSRX talLeftRear(2);
TalonSRX talRghtRear(3);

void initDrive()
{
	/* both talons should blink green when driving forward */
	talRghtFrnt.SetInverted(true);
	talRghtRear.SetInverted(true);
}

void driveLeftFrnt(double fwd)
{
	talLeftFrnt.Set(ControlMode::PercentOutput, fwd);
}
void driveRghtFrnt(double fwd)
{
	talRghtFrnt.Set(ControlMode::PercentOutput, fwd);
}
void driveLeftRear(double fwd)
{
	talLeftRear.Set(ControlMode::PercentOutput, fwd);
}
void driveRghtRear(double fwd)
{
	talRghtRear.Set(ControlMode::PercentOutput, fwd);
}
...
int main() {
	...
	while (true) {
		...
		while (true) {
			...
			double y = ((double)SDL_JoystickGetAxis(joy, 1)) / -100000.0;
